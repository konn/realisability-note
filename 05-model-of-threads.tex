...
\begin{notation}
 $V$において$\mathbf{n} \defeq \set{0, s0, \dots, s^{n-1} 0}$として，$\gimel : \Set{ s^n 0 | n < \omega } \to \hat{V}$を次で定める：
 \[
...
\begin{lemma}\label{lem:gimel-n-props}
 ...
 \begin{enumerate}
  \item \label{item:lt-elem}$\forall m \strin \hat{\N}\: \forall n \strin \hat{\N}\: \left[ \braket{m < n} = 1 \iff m \strin \gimel n\right]$,
  \item \label{item:lt-str-subs}$\forall m \strin \hat{\N}\: \forall n \strin \hat{\N}\: \left[ \braket{m < n} = 1 \to \gimel m \sqsubseteq \gimel n\right]$,
  \item \label{item:lt-plus}$\forall m \strin \hat{\N}\: \forall n \strin \hat{\N}\: \left[ \braket{m < n} = 1 \iff \exists y \strin \hat{\N}\: n = m + y + 1\right]$.
 \end{enumerate}
 よって特に$\gimel n \simeq \Set{x \strin \hat{\N} | \braket{x < n} = 1}$.
\end{lemma}
\begin{proof}
 \ref{item:lt-elem} $n, m \in \N$を任意にとれば$\|\braket{\hat{m} < \hat{n}} = 1\| = \|\hat{m} \strin \gimel n\|$.
...
\begin{lemma}\label{lem:natural-bij-prod}
 $V^{\mathcal{R}}$において，写像$(x, y) \mapsto my + x$は$\gimel m \times \gimel n \to \gimel(\mathbf{mn})$への全単射となっている．
 ...
 \begin{enumerate}
  ...
  \item $\forall m,n \strin \hat{\N}\:\forall z \strin \gimel(\mathbf{mn}) \:
...
まず，$\gimel 2^n$は$\Pow(\NN)$に外延的な意味でも単射で埋め込むことが出来る：
\begin{lemma}
 $V^{\mathcal{R}} \models \forall n \strin \NN \: \gimel(\mathbf{2^n}) \eembto \Pow(\R)$.
\end{lemma}
\begin{proof}
 まず\cref{lem:natural-bij-prod}を$n \strin \NN$についての帰納法で繰り返し適用すれば，$\gimel(\mathbf{2^n})$と$(\gimel 2)^n$の間に自然な全単射が存在することがわかる．
...
\end{corollary}
\begin{proof}
 $n < 2^m$なる$m$を取れば，\cref{lem:gimel-n-props}の\ref{item:lt-str-subs}より$\gimel n \sqsubseteq \gimel (\mathbf{2^m}) \eembto \Pow(\R)$.
\end{proof}
